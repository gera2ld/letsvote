...
cur = db.cursor()
cur.executescript('''
DROP TABLE IF EXISTS voting_meta;
...
CREATE UNIQUE INDEX IF NOT EXISTS uniuser ON vote_meta(voting_id, user_id);
CREATE UNIQUE INDEX IF NOT EXISTS unidata ON vote_data(vote_id, option_id);
''')
db.commit()

def mock():
    cur.execute('INSERT INTO voting_meta (title, desc) VALUES (?, ?)', ('Hello', 'Hello world'))
    mid = cur.lastrowid
    for title in [
        '一',
        '二',
        ...
        cur.execute('INSERT INTO voting_options (voting_id, title) VALUES (?, ?)', (mid, title))
    db.commit()
